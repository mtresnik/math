...
open class Rect(val x: Double, val y: Double, val width: Double, val height: Double) {
    ...
    fun toLineSegments(): List<LineSegment<ArrayPoint>> {
        val ret = mutableListOf<LineSegment<ArrayPoint>>()
        ...
        ret.add(LineSegment(topLeft, topRight))
        ret.add(LineSegment(topRight, bottomRight))
        ret.add(LineSegment(bottomRight, bottomLeft))
        ret.add(LineSegment(bottomLeft, topLeft))
        return ret
    }
    ...
    companion object {
        ...
        fun minimize(rects: Collection<Rect>): Pair<Double, Double> =
            rects.map { it.width }.minOrNull()!! to rects.minOf { it.height }

        fun maximize(rects: Collection<Rect>): Pair<Double, Double> =
            rects.map { it.width }.maxOrNull()!! to rects.maxOf { it.height }
