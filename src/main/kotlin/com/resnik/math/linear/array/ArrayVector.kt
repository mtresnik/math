...
open class ArrayVector(vararg val values: Double) {
    ...
    operator fun plus(other: ArrayPoint): ArrayPoint =
        ArrayPoint(*values.zip(other.values) { a, b -> a + b }.toDoubleArray())

    operator fun plus(other: ArrayVector): ArrayVector =
        ArrayVector(*values.zip(other.values) { a, b -> a + b }.toDoubleArray())

    operator fun minus(other: ArrayVector): ArrayVector =
        ArrayVector(*values.zip(other.values) { a, b -> a - b }.toDoubleArray())
    ...
    fun hadamard(other: ArrayVector): ArrayVector =
        ArrayVector(*values.indices.map { values[it] * other.values[it] }.toDoubleArray())
    ...
    fun maxIndex(): Int = values.indices.maxByOrNull { values[it] } ?: -1
