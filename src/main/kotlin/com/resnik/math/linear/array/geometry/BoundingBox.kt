...
open class BoundingBox(vararg points: ArrayPoint) : Shape2d<ConcaveHull> {
    ...
    val minDim: Int = points.minByOrNull { it.dim }?.dim ?: throw IllegalStateException("Dim was null!")
    val minPoint: ArrayPoint
    val maxPoint: ArrayPoint
    val _points: MutableList<ArrayPoint> = mutableListOf()

    init {
        minPoint = ArrayPoint(*DoubleArray(minDim) { index -> points.minByOrNull { it[index] }!![index] })
        maxPoint = ArrayPoint(*DoubleArray(minDim) { index -> points.maxByOrNull { it[index] }!![index] })
