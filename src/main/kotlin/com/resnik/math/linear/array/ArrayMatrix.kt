...
class ArrayMatrix {
    ...
    fun sum(): Double = values.sumOf { it.sum() }

    operator fun plus(other: ArrayMatrix): ArrayMatrix =
        ArrayMatrix(values.zip(other.values) { col1, col2 -> col1.zip(col2) { x, y -> x + y }.toDoubleArray() }
            .toTypedArray())

    operator fun minus(other: ArrayMatrix): ArrayMatrix =
        ArrayMatrix(values.zip(other.values) { col1, col2 -> col1.zip(col2) { x, y -> x - y }.toDoubleArray() }
            .toTypedArray())
    ...
    fun setFrom(other: ArrayMatrix) = values.indices.forEach { row ->
    ...
    fun hadamard(other: ArrayMatrix): ArrayMatrix =
        ArrayMatrix(values.zip(other.values) { col1, col2 -> col1.zip(col2) { x, y -> x * y }.toDoubleArray() }
            .toTypedArray())

    fun apply(func: Function1<Double, Double>): ArrayMatrix =
        ArrayMatrix(values.map { it.map(func).toDoubleArray() }.toTypedArray())
