...
class Plot2d(private val colorMap : MutableMap<MutableList<ArrayPoint2d>, Color> = mutableMapOf(),
             width : Int = 500,
             height : Int = 500,
             padding : Int = 100,
    ...
    fun getX(xVal : Double) : Int = (width * (xVal - boundingBox().minX()) / (boundingBox().maxX() - boundingBox().minX())).toInt() + padding

    fun getY(yVal : Double) : Int = height - (height * (yVal - boundingBox().minY()) / (boundingBox().maxY() - boundingBox().minY())).toInt() + padding
